...
public:
    ...
    template <class F> void map(F &f) {
        if (all) {
            cilk_for(uint64_t i = 0; i < max_el; i++) { f.update(i); }
            return;
        }
        if (is_sparse) {
            const uint32_t start = queue->shared_out_start;
    ...
    VertexSubset(bool *els, uint64_t len)
        ...
        _Pragma("cilk grainsize = 256") cilk_for(uint64_t i = 0; i < max_el; i++) {
            if (els[i]) {
                ba->set(i);
            }
        }
    ...
    void finalize() {
        if (is_sparse) {
            //queue->reset();
            _Pragma("cilk grainsize = 1") cilk_for(int i = 0; i < getWorkers(); i++) {
    ...
    void convert_to_sparse() {
        ...
        cilk_for(uint32_t i = 0; i < max_el; i++) {
            if (ba->get(i)) {
                queue_array[4 * getWorkerNum()].push_back(i);
            }
        }
        cilk_for(int i = 0; i < getWorkers(); i++) {
            queue_array[i * 4].flush();
        }
        queue->slide_window();
    }
