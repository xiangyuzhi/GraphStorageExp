...
void run_algorithm(commandLine& P) {
  ...
  auto filename = P.getOptionValue("-f", "none");
    ...
    // read edges from input graph data
...
//#endif
  ...
  for (size_t us=0; us<update_sizes.size(); us++) {
    ...
    if (update_sizes[us] <= 10000000) { // if ntrials less than 1e7 repeat 20 times
      n_trials = 20;
    }
    else { // if ntrials larger than 1e7 just repeat 5 times
      n_trials = 5;
    }
