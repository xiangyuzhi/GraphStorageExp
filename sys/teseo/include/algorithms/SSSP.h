...
void SSSP(OpenMP &openmp, long start) {
    ...
    while(frontier.not_empty()) { // while frontier is not empty
        ...
        VertexSubset output = edgeMap(openmp, frontier, BF_F(ShortestPathLen,Visited), true, true);
        vertexMap(openmp, output,BF_Vertex_F(Visited));
        frontier.del();
        frontier = output;
